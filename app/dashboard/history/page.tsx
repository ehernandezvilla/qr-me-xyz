...
import URLAnalytics from "@/app/components/URLAnalytics";
...
export default function HistoryPage() {
  ...
  const openAnalytics = (shortUrl: string) => {
    setSelectedUrlForAnalytics(shortUrl);
  };
    ...
    <>
      <div className="p-6 bg-white dark:bg-gray-900 rounded shadow">
            ...
            <table className="min-w-full table-fixed divide-y divide-gray-200 dark:divide-gray-700 text-sm">
              <thead className="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                <tr>
                  ...
                  <th className="px-4 py-3 text-center w-32">Analytics</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {history.map((item, index) => (
                  <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <button
                        onClick={() => openAnalytics(item.short_url)}
                        className="flex items-center justify-center mx-auto bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1 rounded transition-colors"
                      >
        ...
        {selectedUrlForAnalytics && (
          <URLAnalytics shortUrl={selectedUrlForAnalytics} onClose={closeAnalytics} />
        )}
      </div>
    </>
  );
